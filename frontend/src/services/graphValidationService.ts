...
export function validateGraph(graph: AnalyzerGraph): string[] {
    ...
    // 2. CORRECTION : Comparer tous les nœuds du graphe avec les nœuds du chemin
    const orphanNodes = [...allNodeIds].filter(id => !pathNodeIds.has(id));
    if (orphanNodes.length > 0) {
        const orphanLabels = orphanNodes.map(id => `"${nodeMap.get(id)?.data.label || nodeMap.get(id)?.data.name}"`).join(', ');
        issues.push(`Les nœuds suivants ne sont pas connectés au pipeline principal : ${orphanLabels}.`);
    }

    // 3. Vérifier les paramètres de chaque nœud sur le chemin

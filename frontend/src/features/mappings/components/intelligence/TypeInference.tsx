...
interface TypeInferenceProps {
  ...
  onInferenceComplete?: (inferredFields: InferredType[]) => void;
}
...
export function TypeInference({ sampleData, onTypesApplied, onInferenceComplete }: TypeInferenceProps) {
  ...
  const inferenceMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Appeler le callback pour exposer les types inférés
      onInferenceComplete?.(data.inferred_types);
    },
    onError: (error) => {

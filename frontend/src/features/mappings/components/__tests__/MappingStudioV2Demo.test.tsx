...
const { schemaMock, toastsFactory, abortableFactory } = vi.hoisted(() => {
  return {
    schemaMock: {
      schema: { version: '2.2' },
    ...
    toastsFactory: () => ({ success: vi.fn(), error: vi.fn(), info: vi.fn() }),
    abortableFactory: () => ({
...
vi.mock('../../hooks/useSchema', () => ({ useSchema: () => schemaMock }));
vi.mock('../../hooks/useToasts', () => ({ useToasts: toastsFactory }));
vi.mock('../../hooks/useShortcuts', () => ({ useShortcuts: () => {} }));
vi.mock('../../hooks/useAbortable', () => ({ useAbortable: abortableFactory }));
...
vi.mock('../ToastsContainer', () => ({ ToastsContainer: () => <div data-testid="toasts" /> }));
vi.mock('../SchemaBanner', () => ({ SchemaBanner: () => <div data-testid="schema-banner" /> }));
...
async function loadComponent() {
  ...
  const mod = await import('../MappingStudioV2Demo'); // import dynamique

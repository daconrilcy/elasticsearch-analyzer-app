...
// --- Icônes pour les panneaux de l'éditeur d'analyseur (votre code existant) ---
...
export const IconSidebar: React.FC = () => {
    // Connexion directe au store pour gérer la page et les panneaux
    const { activePage, setActivePage, activePanel, togglePanel } = useUIStore();

    return (
        <nav className="icon-sidebar">
            {/* La structure de votre sidebar est conservée */}
            <ul>
                {/* --- Section de Navigation Principale --- */}
                <li>
                    <button 
                        className={`nav-button ${activePage === 'analyzer' ? 'active' : ''}`}
                        onClick={() => setActivePage('analyzer')}
                        title="Éditeur d'analyseur"
                    >
                        <AnalyzerIcon />
                    </button>
                </li>
                <li>
                    <button 
                        className={`nav-button ${activePage === 'importer' ? 'active' : ''}`}
            ...
            {/* --- Section Contextuelle (uniquement pour l'éditeur d'analyseur) --- */}
            {activePage === 'analyzer' && (
                <ul>
